...
if ($current_subject=='all')
{
    ...
    for($loop2=0;$loop2!=$loop;$loop2++) {
        ...
        echo "<div data-guide-selected='false' data-guide-id='".$guides_array['id'][$loop2]."' class='col-xs-6 selected-guide'><a href='display-guide.php?guide=".$guides_array['id'][$loop2]."'><button class='btn edit-gudie-btn' style='background:#29d846;color:#fff'>לצפיה במדריך</button></a><button class='btn edit-gudie-btn' onclick='markForDelete(this)'>סמן מדריך</button><button class='btn edit-gudie-btn' onclick=window.open('dashboard.php?dash=edit-guide&guide=".$guides_array['id'][$loop2]."')>ערוך מדריך</button><img src='".$guides_array['guide_images_array_fix'][$loop2][0]."' class='img-responsive  pull-left' style='max-width:177px;height:96px'>";
    ...
    //echo "<button  onclick=elements_to_remove('guides')>אשר</button>";
    ...
    echo "<button type='button' class='btn btn-danger btn-info btn-lg remove_selected_guides' data-toggle='modal' data-target='#myModal'>מחק את המדריכים המסומנים</button>";
